...
HRESULT CPushPinDesktop::SetMediaType(const CMediaType *pMediaType)
{
	...
	if(SUCCEEDED(hr))
	{
	  ...
	  m_rBoundingBox.right = m_rBoundingBox.left + pvi->bmiHeader.biWidth; // allow them to set whatever "scaling size" they want [set m_rScreen is negotiated right here]
	  m_rBoundingBox.bottom = m_rBoundingBox.top + pvi->bmiHeader.biHeight;
